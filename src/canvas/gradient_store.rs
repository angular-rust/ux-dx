...
use crate::canvas::{
    image::ImageStore, paint::MultiStopGradient, Color, ErrorKind, ImageFlags, ImageId, ImageInfo,
...
impl GradientStore {
    ...
    pub fn lookup_or_add<R: Renderer>(
        ...
        if let Some(gradient_image_id) = self.prev_frame.remove(&colors) {
            ...
            let info = ImageInfo::new(
                ...
                256,
                1,
                crate::canvas::PixelFormat::Rgba8,
            );
    ...
    pub fn release_old_gradients<R: Renderer>(
        &mut self,
        images: &mut ImageStore<R::Image>,
        renderer: &mut R,
    ) {
