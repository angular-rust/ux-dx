//! The [NanoVG](https://github.com/memononen/nanovg) API loosely modeled on the
//! [HTML5 Canvas API](https://bucephalus.org/text/CanvasHandbook/CanvasHandbook.html).
//!
//! This is a femtovg adapted for this library.
...
use text::{FontTexture, RenderMode, RenderedGlyph, RenderedGlyphId, TextContextImpl};

mod image;
use self::image::ImageStore;
pub use self::image::{ImageFilter, ImageFlags, ImageId, ImageInfo, ImageSource, PixelFormat};
    ...
    T: Renderer,
{
    ...
    pub fn set_size(&mut self, width: u32, height: u32, dpi: f32) {
        ...
        self.append_cmd(Command::new(CommandType::SetRenderTarget(
            RenderTarget::Screen,
        )));
    }
    ...
    pub fn save(&mut self) {
        let state = self
            .state_stack
            .last()
            .map_or_else(State::default, |state| *state);
    ...
    pub fn filter_image(
        ...
        let mut cmd = Command::new(CommandType::RenderFilteredImage {
            target_image,
            filter,
        });
        ...
        self.verts
            .push(Vertex::new(quad_x0, quad_y0, texture_x0, texture_y0));
        self.verts
            .push(Vertex::new(quad_x1, quad_y1, texture_x1, texture_y1));
        self.verts
            .push(Vertex::new(quad_x1, quad_y0, texture_x1, texture_y0));
        self.verts
            .push(Vertex::new(quad_x0, quad_y0, texture_x0, texture_y0));
        self.verts
            .push(Vertex::new(quad_x0, quad_y1, texture_x0, texture_y1));
        self.verts
            .push(Vertex::new(quad_x1, quad_y1, texture_x1, texture_y1));
    ...
    pub fn fill_path(&mut self, path: &mut Path, mut paint: Paint) {
        ...
        let fringe_width = if paint.anti_alias() {
            self.fringe_width
        } else {
            0.0
        };
        ...
        // GPU uniforms
        let flavor = if path_cache.contours.len() == 1
            && path_cache.contours[0].convexity == Convexity::Convex
        {
        ...
        if let PaintFlavor::Image { id, .. } = paint.flavor {
            cmd.image = Some(id);
        } else if let Some(paint::GradientColors::MultiStop { stops }) =
            paint.flavor.gradient_colors()
        {
            cmd.image = self
                .gradients
    ...
    pub fn stroke_path(&mut self, path: &mut Path, mut paint: Paint) {
        ...
        let fringe_with = if paint.anti_alias() {
            self.fringe_width
        } else {
            0.0
        };
        ...
        if paint.font_size > 92.0 {
            ...
            for cmd in &cmds {
                ...
                for quad in &cmd.quads {
                    let (p0, p1) =
                        transform.transform_point(quad.x0 * invscale, quad.y0 * invscale);
                    let (p2, p3) =
                        transform.transform_point(quad.x1 * invscale, quad.y0 * invscale);
                    let (p4, p5) =
                        transform.transform_point(quad.x1 * invscale, quad.y1 * invscale);
                    let (p6, p7) =
                        transform.transform_point(quad.x0 * invscale, quad.y1 * invscale);
