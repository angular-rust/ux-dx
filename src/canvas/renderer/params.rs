use crate::canvas::{
    paint::GradientColors, Color, ImageFlags, ImageStore, Paint, PaintFlavor, PixelFormat, Scissor,
    Transform2D,
};
    ...
    pub(crate) fn new<T>(
        ...
        let (scissor_ext, scissor_scale) = if let Some(ext) = scissor.extent {
            if ext[0] < -0.5 || ext[1] < -0.5 {
                ...
                let scissor_scale = [
                    (scissor.transform[0] * scissor.transform[0]
                        + scissor.transform[2] * scissor.transform[2])
                        .sqrt()
                        / fringe_width,
                    (scissor.transform[1] * scissor.transform[1]
                        + scissor.transform[3] * scissor.transform[3])
                        .sqrt()
                        / fringe_width,
                ];
        ...
        params.has_mask = if paint.alpha_mask().is_some() {
            1.0
        } else {
            0.0
        };
        ...
        match paint.flavor {
            ...
            PaintFlavor::LinearGradient {
                ...
                let mut transform =
                    Transform2D([dy, -dx, dx, dy, start_x - dx * large, start_y - dy * large]);
