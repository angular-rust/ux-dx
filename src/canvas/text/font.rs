use fnv::FnvHashMap;
use owned_ttf_parser::{AsFaceRef, Face as TtfFont, GlyphId, OwnedFace};

use crate::canvas::{ErrorKind, Path};
...
impl Font {
    pub fn new(data: &[u8]) -> Result<Self, ErrorKind> {
        let owned_ttf_font =
            OwnedFace::from_vec(data.to_owned(), 0).map_err(|_| ErrorKind::FontParseError)?;
