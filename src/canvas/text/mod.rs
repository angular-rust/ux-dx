...
/// returned.
///
/// The [`FontId`] can be supplied to [`crate::canvas::Paint`] along with additional parameters
...
/// parameter. If you need measurements that take a [`crate::canvas::Canvas`]'s transform or dpi into
/// account (see [`crate::canvas::Canvas::set_size()`]), you need to use the measurement functions
/// on the canvas.
#[derive(Clone)]
...
pub(crate) fn render_atlas<T: Renderer>(
    ...
    for glyph in &text_layout.glyphs {
        let subpixel_location = crate::canvas::geometry::quantize(glyph.x.fract(), 0.1) * 10.0;
...
fn render_glyph<T: Renderer>(
    ...
    let (dst_index, dst_image_id, (dst_x, dst_y)) =
        find_texture_or_alloc(canvas, width as usize, height as usize)?;
    ...
    let x_quant = crate::canvas::geometry::quantize(glyph.x.fract(), 0.1);
    ...
    canvas.global_composite_blend_func(
        crate::canvas::BlendFactor::SrcAlpha,
        crate::canvas::BlendFactor::One,
    );
