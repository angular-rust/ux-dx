...
use crate::ext::{
    error::GameResult,
    math::{Position, Region, Size, Transform, Viewport},
};
...
pub use color::Color;
pub use font::Font;
pub use opengl::{Filter, FilterMode, PrimitiveType, Wrap, WrapMode};
pub use params::{MeshDrawParams, SpriteDrawParams, TextDrawParams, TextLayoutGravity};
...
const SPRITE_ELEMENTS: [u16; SPRITE_ELEMENT_COUNT] = [0, 2, 1, 1, 2, 3];
...
impl Graphics {
    pub(crate) fn new(graphics_config: GraphicsConfig, window: Rc<Window>) -> GameResult<Self> {
        ...
        let projection_matrix =
            cgmath::ortho(0.0, logical_size.width, logical_size.height, 0.0, -1.0, 1.0);
        ...
        program.bind();
        program.set_uniform_matrix_4(
            "u_projection",
            AsRef::<[f32; 16]>::as_ref(&projection_matrix),
        );

        let default_texture = Texture::white_1_1()?;

        let max_texture_size = gl::get_integerv(GL_MAX_TEXTURE_SIZE) as u32;
    ...
    pub(crate) fn resize(&mut self, physical_size: PhysicalSize<u32>, scale_factor: f64) {
        ...
        if self.canvas.is_none() {
            ...
            self.projection_matrix =
                cgmath::ortho(0.0, logical_size.width, logical_size.height, 0.0, -1.0, 1.0);
            self.program.set_uniform_matrix_4(
                "u_projection",
                AsRef::<[f32; 16]>::as_ref(&self.projection_matrix),
            );
        }
    }
    ...
    pub fn flush(&mut self) {
        if !self.vertices.is_empty() && !self.elements.is_empty() {
            ...
            self.renderer
                .draw_elements(self.draw_command.primitive, self.elements.len(), 0);
        ...
        if self.viewport != viewport {
            ...
            if self.canvas.is_some() {
                ...
                let physical_viewport = {
                    let physical_position = LogicalPosition::new(self.viewport.x, self.viewport.y)
                        .to_physical::<i32>(scale_factor);
                    let physical_size = LogicalSize::new(self.viewport.width, self.viewport.height)
                        .to_physical::<i32>(scale_factor);
                    Viewport::new(
                        physical_position.x,
                        physical_position.y,
                ...
                let physical_size = {
                    let physical_size = LogicalSize::new(self.size.width, self.size.height)
                        .to_physical::<i32>(scale_factor);
    ...
    pub fn set_canvas(&mut self, canvas: Option<&Canvas>) {
        ...
        if self.canvas != canvas {
            ...
            if let Some(canvas_size) = canvas_size {
                ...
                gl::viewport(0, 0, canvas_size.width as i32, canvas_size.height as i32);

                self.projection_matrix =
                    cgmath::ortho(0.0, self.size.width, 0.0, self.size.height, -1.0, 1.0);
            } else {
    ...
    fn append_vertices_and_elements(&mut self, vertices: Vec<Vertex>, elements: Option<Vec<u16>>) {
        ...
        if self.renderer.vertex_size() < self.vertices.len() + vertices.len()
            || self.renderer.element_size() < self.elements.len() + elements.len()
        {
            self.flush();
        }
        assert!(
            self.renderer.vertex_size() >= self.vertices.len() + vertices.len(),
            "no enough renderer vertex size"
        );
        assert!(
            self.renderer.element_size() >= self.elements.len() + elements.len(),
            "no enough renderer element size"
        );
        let append_vertex_count = vertices.len() as u16;
        let element_offset = self.vertices.len() as u16;
        for element in &mut elements {
            assert!(
                *element < append_vertex_count,
                "element must < append vertex count"
            );
    ...
    pub fn draw_mesh<'a>(
        &mut self,
        texture: impl Into<TextureRef<'a>>,
        params: impl Into<Option<MeshDrawParams>>,
        transform: impl Into<Option<Transform>>,
    ) {
        ...
        let vertices = params
            ...
            .map(|mut vertices| {
                for vertex in &mut vertices {
                    vertex.position = {
                        let position =
                            matrix * Vector4::new(vertex.position.x, vertex.position.y, 0.0, 1.0);
    ...
    pub fn draw_sprite<'a>(
        &mut self,
        texture: impl Into<TextureRef<'a>>,
        params: impl Into<Option<SpriteDrawParams>>,
        transform: impl Into<Option<Transform>>,
    ) {
        let texture = texture.into();
        let params = params.into().unwrap_or_default();
        ...
        let region = params
            .region
            .unwrap_or_else(|| Region::new(0.0, 0.0, texture_size.width, texture_size.height));
        let origin = params.origin.unwrap_or_else(|| Position::zero());
        let matrix = self.transform_matrix * transform.0;
        let x0y0 = matrix * Vector4::new(-origin.x, -origin.y, 0.0, 1.0);
        let x1y0 = matrix * Vector4::new(-origin.x + region.width, -origin.y, 0.0, 1.0);
        let x0y1 = matrix * Vector4::new(-origin.x, -origin.y + region.height, 0.0, 1.0);
        let x1y1 = matrix
            * Vector4::new(
        ...
        let colors = params
            .colors
            .unwrap_or_else(|| [Color::WHITE, Color::WHITE, Color::WHITE, Color::WHITE]);
    ...
    pub fn draw_text(
        ...
        font: &Font,
        text: &str,
        params: impl Into<Option<TextDrawParams>>,
        transform: impl Into<Option<Transform>>,
    ) {
        let params = params.into().unwrap_or_default();
        let transform = transform.into().unwrap_or_default();
        ...
        let (line_layout_infos, layout_height) = {
            ...
            for c in text.chars() {
                if c.is_control() {
                    ...
                    if wrap_width > 0.0
                        && caret.x > 0.0
                        && caret.x + glyph_metrics.advance_width > wrap_width
                    {
                        line_layout_infos.push((glyph_positions, caret.x));
                        glyph_positions = Vec::new();
                        caret.x = 0.0;
                    ...
                    match font.cache_glyph(c, text_size, hidpi_scale_factor) {
                        Ok(cached_by) => {
                            ...
                            let glyph_position = Position::new(
                                ...
                                glyph_position.y
                                    + line_metrics.ascent
                                    + draw_info.uv_bounds.min_y()
                                    + (line_height - line_metrics.height) / 2.0,
                            );
                            let x0y0 = matrix
                                * Vector4::new(
                                    offset_x + glyph_position.x,
                                    offset_y + glyph_position.y,
                                    0.0,
                                    1.0,
                                );
                            let x1y0 = matrix
                                * Vector4::new(
                                    offset_x + glyph_position.x + draw_info.uv_bounds.width,
                                    offset_y + glyph_position.y,
                                    0.0,
                                    1.0,
                                );
                            let x0y1 = matrix
                                * Vector4::new(
                                    offset_x + glyph_position.x,
                                    offset_y + glyph_position.y + draw_info.uv_bounds.height,
                                    0.0,
                                    1.0,
                                );
                            let x1y1 = matrix
                        ...
                        Err(cache_error) => {
                            let cache_size_maximized =
                                font.cache_texture_size() >= self.max_texture_size;
                            match (cache_error, cache_size_maximized) {
                                ...
                                _ => {
                                    self.flush();
                                    font.resize_cache(
                                        (font.cache_texture_size() * 2).min(self.max_texture_size),
                                    );
                                }
                            }
                        }
