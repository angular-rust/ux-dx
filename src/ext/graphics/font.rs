use super::{opengl, Filter, Graphics, Texture};
use crate::ext::{
    engine::Engine,
...
impl Font {
    ...
    pub(crate) fn line_metrics(&self, px: f32) -> LineMetrics {
        let horizontal_line_metrics = self
            .font
    ...
    pub(crate) fn cache_glyph(
        ...
        if glyph_cache_size.width > cache_texture_size
            || glyph_cache_size.height > cache_texture_size
        {
        ...
        for row in cache.rows.iter_mut() {
            if glyph_cache_size.height <= row.height
                && glyph_cache_size.width <= cache_texture_size - row.width
            {
        ...
        if let Some(region) = region {
            ...
            cache
                .texture
                .update_pixels(region, Some(&pixels))
    ...
    pub(crate) fn resize_cache(&self, cache_texture_size: u32) {
        ...
        cache
            .texture
            .resize((cache_texture_size, cache_texture_size));
