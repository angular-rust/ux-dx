...
use cgmath::prelude::*;
use cgmath::{num_traits::clamp, BaseFloat, Matrix4, Quaternion, Vector2, Vector3, Vector4};
...
impl<F: BaseFloat> ArcballCamera<F> {
    ...
    pub fn get_inv_camera(&self) -> Matrix4<F> {
        self.inv_camera
    }
    ...
    pub fn rotate(&mut self, mouse_prev: Vector2<F>, mouse_cur: Vector2<F>) {
        ...
        let mouse_cur_ball = Self::screen_to_arcball(m_cur);
        let mouse_prev_ball = Self::screen_to_arcball(m_prev);
