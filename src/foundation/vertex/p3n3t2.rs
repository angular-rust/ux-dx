...
impl P3N3T2 {
    pub fn new(pos: Point3<f32>, nor: Point3<f32>, uv: Point2<f32>) -> Self {
        Self { pos, nor, uv }
    }

    pub fn from_components(pos: [f32; 3], nor: [f32; 3], uv: [f32; 2]) -> Self {
        Self {
            pos: Point3 {
                x: pos[0],
                y: pos[1],
...
impl VertexAttributesLayout for P3N3T2 {
    fn layout() -> &'static VertexBufferLayout<'static> {
        &VertexBufferLayout {
            array_stride: mem::size_of::<P3N3T2>() as BufferAddress,
