...
impl<F: Font + Sync, H: BuildHasher> GlyphBrush<F, H> {
    ...
    #[inline]
    pub fn draw_queued(&mut self, target_width: u32, target_height: u32) -> Result<(), String> {
    ...
    /// the `render_format` provided on creation of the `GlyphBrush`.
    #[inline]
    pub fn draw_queued_with_transform(&mut self, transform: [f32; 16]) -> Result<(), String> {
        self.process_queued();
    ...
    pub fn draw_queued_with_transform_and_scissoring(
        ...
        self.process_queued();
        self.pipeline.draw(transform, Some(region));
...
impl<F: Font, H: BuildHasher> GlyphBrush<F, H> {
    fn new(raw_builder: glyph_brush::GlyphBrushBuilder<F, H>) -> Self {
        let glyph_brush = raw_builder.build();
        let (cache_width, cache_height) = glyph_brush.texture_dimensions();

        GlyphBrush {
            pipeline: Pipeline::new(cache_width, cache_height),
            glyph_brush,
        }
    }
}
