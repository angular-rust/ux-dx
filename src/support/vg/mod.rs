...
use std::mem;
use vg::{
    renderer::Vertex, BlendFactor, Color, Command, CommandType, CompositeOperationState, ErrorKind,
    FillRule, GlyphTexture, ImageFilter, ImageId, ImageInfo, ImageSource, ImageStore, Params,
    RenderTarget, Renderer, Scissor, ShaderType,
};
...
impl OpenGl {
    ...
    fn set_uniforms(
        ...
        let glyphtex = match glyph_tex {
            GlyphTexture::None => 0,
            GlyphTexture::AlphaMask(id) | GlyphTexture::ColorTexture(id) => {
                images.get(id).map(|tex| tex.id()).unwrap_or(0)
            }
        };

        gl::active_texture(GL_TEXTURE0 + 1);
        gl::bind_texture(GL_TEXTURE_2D, glyphtex);
    ...
    pub fn set_screen_target(&mut self, framebuffer_object: Option<Framebuffer>) {
        match framebuffer_object {
            Some(fbo_id) => self.screen_target = Some(Framebuffer::from_external(fbo_id)),
    ...
    fn render_gaussian_blur(
        ...
        let image_paint = vg::Paint::image(
            cmd.image.unwrap(),
            0.,
            0.,
